...
$this->add_render_attribute('wrap', [
	'class' => [
		...
		(!empty($settings['heading_text']) || !empty($settings['desc'])) ? 'pt-40' : ''
	]
]);
// Feature Desc
...
$this->add_render_attribute('edesc',[
	'class' => [
		'cms-eprocess-desc',
...
?>
<h4 <?php ctc_print_html($this->get_render_attribute_string('eheading')); ?>><?php 
	echo nl2br($settings['heading_text']);
?></h4>
<div <?php ctc_print_html($this->get_render_attribute_string('edesc')); ?>><?php 
	...
	<?php 
		...
		foreach ( $accordions as $key => $value ):
			...
			$this->add_render_attribute( $item_title_key, [
				'class' => [ 
					'cms-accordion-title text-md text-active-lg font-700',
					'cursor-pointer',
					$is_active ? 'active' : '',
